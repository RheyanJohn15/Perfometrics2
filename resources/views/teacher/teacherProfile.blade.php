...
<html :class="{ 'theme-dark': dark }" x-data="data()" lang="en">
  <head>
    ...
    <title>Teacher Profile- {{$teacher_username}}</title>
    <link
            ...
            <div class="flex flex-col md:flex-row w-screen mt-4">
                ...
                <div class="md:w-1/2 h-full mr-4 px-4 py-3 bg-white rounded-lg shadow-md dark:bg-gray-800">
                  <div class="profile">
                   ...
                   <div class="profileImage" style="background-image:url('{{$profilePic}}')">
  ...
  'form-input'
        ...
        class="px-4 mt-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple"
      >
      <i class="fa-solid fa-envelope-circle-check"></i> Change Recovery Email <strong>({{$teacher_mail}})</strong> 
      </button>
...
<div class="profileHolder">
  ...
  <form id="uploadForm" method="post" action="{{route('updatePictureTeacher')}}" enctype="multipart/form-data">
    @csrf
    @method('post')
...
function setLoading(){
  ...
  <div id="changeRecoveryEmail">
     <div id="confirmationPassword" >
      ...
      >
        <button
          @click="closeModal" type="button" onclick="confirmPasswordRecovery()"
     ...
     <form method='post' id="newRecoveryEmail" style="display: none">
      ...
      <div class="mt-4 mb-6">
        ...
        <label class="block text-sm">
          ...
          <input type="hidden" name="teacher_id" value="{{session('user_id')}}">
          <input
            class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
...
$(document).ready(function() {
            $("form#newRecoveryEmail").submit(function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('changeRecoveryMailTeacher')}}", // Replace with the URL where you want to process the form data
                    data: formData,
     ...
     $.ajax({
         type: 'POST',
         url: "{{route('confirmChangeMailTeacher')}}",
         data: formData,
         success: function(response) {

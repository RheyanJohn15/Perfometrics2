...
<html :class="{ 'theme-dark': dark }" x-data="data()" lang="en">
  <head>
    ...
    <title>Teacher- {{$teacher_username}}-Remarks</title>
    <link
  ...
  <body>
    ...
    >
      ...
      <div class="flex flex-col flex-1 w-full">
        ...
        <main class="h-full overflow-y-auto">
          ...
          <h2
           ...
           Evaluation Results
            ...
            @php
            $remarksStudent = App\Models\EvaluationResult::select('evaluator_id', 'evaluation_remarks')
                ->where('teacher_id', session('user_id'))
                ->where('evaluator_type', 'Student')
                ->distinct()
                ->get();

            $remarksCoordinator = App\Models\EvaluationResult::select('evaluator_id', 'evaluation_remarks')
                ->where('teacher_id', session('user_id'))
                ->where('evaluator_type', 'Coordinator')
                ->distinct()
                ->get();
                $count=1;
        @endphp
        
        @foreach ($remarksStudent as $rem)
            <div style="padding: 20px;" class="mb-4 border border-solid flex">
                <h3 class="font-semibold text-lg">Student #{{$count}}--> </h3>
                <span>{{$rem->evaluation_remarks}}</span>
            </div>
            @php
                $count++;
            @endphp
        ...
        @foreach ($remarksCoordinator as $rem)
        <div style="padding: 20px;" class="mb-4 border border-solid flex">
            <h3 class="font-semibold text-lg">Coordinator #{{$count}}--> </h3>
            <span>{{$rem->evaluation_remarks}}</span>
        </div>
        @php
            $count++;

@if (session()->has('user_id') && session('user_type')==="Coordinator")
@php
    ...
    if($image->coordinator_suffix==="none"){
      $coordinatorFinalSuffix= "";
    }else{
      $coordinatorFinalSuffix= $image->coordinator_suffix;
    }
    $coordinator_username= $image->coordinator_first_name. " ". substr($image->coordinator_middle_name, 0, 1). '. '. $image->coordinator_last_name. " ". $coordinatorFinalSuffix;
    @endphp
<!DOCTYPE html>
<html :class="{ 'theme-dark': dark }" x-data="data()" lang="en">
  <head>
    ...
    <title>Coordinator Profile- {{$coordinator_username}}</title>
    <link
...
'form-input'
        ...
        class="px-4 mt-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple"
      >
      <i class="fa-solid fa-envelope-circle-check"></i> Change Recovery Email <strong>({{$coordinator_mail}})</strong> 
      </button>
...
<div class="profileHolder">
  ...
  <form id="uploadForm" method="post" action="{{route('updateProfilePicCoordinator')}}" enctype="multipart/form-data">
    @csrf
    @method('post')
...
function setLoading(){
  ...
  <div id="changeRecoveryEmail" style="display: none">
     ...
     <form method="post" id="newRecoveryEmail" style="display: none">
      @csrf
      @method('post')
      <div class="mt-4 mb-6">
        <!-- Modal title -->
        ...
        <label class="block text-sm">
          ...
          <input type="hidden" name="coordinator_id" value="{{session('user_id')}}">
          <input
            class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
    ...
    <button type="submit" id="confirmButton" onclick="onSpinner()"
      class="w-full px-5 py-3 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg sm:w-auto sm:px-4 sm:py-2 active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple"
    >
...
$(document).ready(function() {
            $("form#newRecoveryEmail").submit(function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('changeRecoveryMailCoordinator')}}", // Replace with the URL where you want to process the form data
                    data: formData,
     ...
     $.ajax({
         type: 'POST',
         url: "{{route('confirmChangeMailCoordinator')}}",
         data: formData,
         success: function(response) {

...
<html :class="{ 'theme-dark': dark }" x-data="data()" lang="en">
  <head>
    ...
    <title>Coordinator- {{$coordinator_username}} </title>
    <link
  ...
  <body>
     ...
     @if($image->coordinator_status ===0)
     @include('coordinator/newUser', ['coordinator_id'=>$image->id, 'name'=> $image->coordinator_first_name])
     @else
...
<script>
    ...
    if (selectedValue !== null) {
      axios.get("{{route('evaluationCheck')}}?teacher="+selectedValue+"&student_id={{session('user_id')}}&type=Coordinator")
